...
//		member.setMemMail("test@test.com");
...
//		member.setMemAdd2("오류");
		...
		Set<ConstraintViolation<MemberVO>> constrainViolation = validator.validate(member,InsertGroup.class);
	
		if(constrainViolation.isEmpty()) { //검증 통과
			System.out.println("검증 통과");
			
		} else {	// 검증 불통
			Map<String, List<String>> errors = new HashMap<>();
			constrainViolation.stream()
					.forEach((cv)->{	//프로퍼티 한건의 검증결과
